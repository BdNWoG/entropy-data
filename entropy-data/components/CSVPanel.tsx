...
const isNumeric = (val: unknown): val is number => {
  ...
  return !isNaN(n) && (val as string).trim() !== "";
};

const convertCell = (cell: string): string | number => {
  return isNumeric(cell) ? parseFloat(cell) : cell;
};
...
const sortAndGroupByDate = (data: (string | number)[][]): (string | number)[][] => {
  ...
  body.forEach((row) => {
    const d = typeof row[0] === "string" ? standardizeDate(row[0]) : "";
    if (!d) return;
    if (!grouped[d]) grouped[d] = new Array(row.length - 1).fill(0);

    row.slice(1).forEach((v, i) => {
      if (isNumeric(v)) {
        grouped[d][i] = (isNumeric(grouped[d][i]) ? (grouped[d][i] as number) : 0) + (v as number);
      } else {
        grouped[d][i] = grouped[d][i] ?? v;
      }
    });
  });
...
const CSVPanel: React.FC<CSVPanelProps> = ({ setPlotData }) => {
  ...
  // state & refs
  // ────────────────────────────────────────────────
  const [editableData, setEditableData] = useState<(string | number)[][] | null>(null);
  ...
  const handleCSVUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      complete: ({ data }) => {
        const parsed = (data as string[][]).map((row, ri) =>
          row.map((cell, ci) => (ci === 0 && ri !== 0 ? standardizeDate(cell) : convertCell(cell)))
        );
        const csv = sortAndGroupByDate(parsed);
        setEditableData(csv);
        setView("table");
      },
  ...
  // ────────────────────────────────────────────────
  const handleAddRow = () =>
    editableData && setEditableData([...editableData, new Array(editableData[0].length).fill("")]);
  ...
  const handleDeleteColumn = (idx: number) =>
    editableData &&
    editableData[0].length > 1 &&
    setEditableData(editableData.map((r) => r.filter((_, i) => i !== idx)));
  const handleReorderRows = (src: number, tgt: number) =>
    editableData &&
    setEditableData((prev) => {
      const d = [...prev!];
      const [r] = d.splice(src, 1);
      d.splice(tgt, 0, r);
      return d;
    });
  const handleReorderCols = (src: number, tgt: number) =>
    editableData &&
    setEditableData((prev) =>
      prev!.map((r) => {
        const row = [...r];
        const [c] = row.splice(src, 1);
        row.splice(tgt, 0, c);
        return row;
      })
    );
  ...
  const handleDragStart = (idx: number, type: "row" | "column") => {
    setDraggedIndex(idx);
  ...
  useEffect(() => {
    ...
    const timestamps = rows.map((r) => (typeof r[0] === "string" ? standardizeDate(r[0]) : ""));
    const plot: PlotData = {};

    header.slice(1).forEach((h, ci) => {
      const values = rows.map((r) => r[ci + 1]);
      const isColumnNumeric = values.every((v) => isNumeric(v));
      if (!isColumnNumeric) return; // skip non‑numeric columns
      plot[h] = {
  ...
  return (
    <div
      ...
      style={{ height: "450px", width: "100%", maxWidth: "50vw" }}
    >
      {/* Initial */}
      {view === "initial" ? (
        <div className="h-full flex flex-col items-center justify-center gap-4">
          <button
            ...
            className="bg-borderBlue text-white px-6 py-3 rounded-md hover:bg-blue-600 transition-colors"
          >
            Import by API
          </button>
          <label
          ...
          <button
            onClick={() => {
              const blank = Array.from({ length: 3 }, () => new Array(3).fill(""));
        ...
        <div className="relative h-full flex flex-col">
          <div ref={buttonRowRef} className="flex gap-2 mb-4 flex-wrap">
            ...
            <button
              onClick={() => editableData && setEditableData(sortAndGroupByDate(editableData))}
          ...
          <div
            ...
            style={{ height: `${tableHeight}px`, overflowY: "auto", overflowX: "auto" }}
          >
                  ...
                  <tr>
                    {editableData?.[0].map((h, ci) => (
                      <th
                        ...
                        draggable
                        onDragStart={() => handleDragStart(ci, "column")}
                ...
                <tbody>
                  {editableData?.slice(1).map((row, ri) => (
                    <tr
                      ...
                      draggable
                      onDragStart={() => handleDragStart(ri, "row")}
                      onDragOver={(e) => e.preventDefault()}
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEditableData((d) => {
                                ...
                                if (ci === 0) {
                                  newVal = standardizeDate(val);
                                } else {
                                  newVal = isNumeric(val) ? parseFloat(val) : val;
                                }
                                nd[ri + 1][ci] = newVal;
                                return nd;
                              })
      ...
      {isApiModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="relative bg-white text-black rounded-lg shadow-lg p-8 w-[80%] max-w-md">
            ...
            <div className="mb-4">
              <label htmlFor="api-input" className="block text-sm font-medium text-gray-700 mb-2">
                API Endpoint, Dune Query ID, or CSV Text:
              </label>
              <textarea
                id="api-input"
                rows={4}
                ...
                placeholder="1234567  (Dune query ID)   OR   https://example.com/data.csv   OR   paste CSV here"
              />
            </div>
