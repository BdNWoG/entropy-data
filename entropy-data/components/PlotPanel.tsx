...
const PlotPanel: React.FC<PlotPanelProps> = ({ plotData, customization, plotRef }) => {
  ...
  useEffect(() => {
    ...
    } else if (plotData === null && internalPlotRef.current) { // Ensure we only clear when plotData is null
        Plotly.purge(internalPlotRef.current); // Fully clear the plot
        internalPlotRef.current.innerHTML = ""; // Ensure the div is empty
    }
  ...
  return (
    <div className="flex-1 bg-panel border-2 border-borderBlue rounded-xl shadow-lg p-4 box-border">
      {plotData != null ? (
        <div
          ref={internalPlotRef}
          key={plotData ? "plot-present" : "plot-null"} // Force re-render by changing the key
